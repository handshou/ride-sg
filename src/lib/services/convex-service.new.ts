...
class ConvexServiceImpl implements ConvexService {
  ...
  isConfigured(): Effect.Effect<boolean> {
    return Effect.succeed(
      this.config.convex.publicUrl !== "" &&
        this.config.convex.publicUrl !== undefined,
    );
  }
...
/**
 * ConvexService as Effect.Service
...
export class ConvexService extends Effect.Service<ConvexService>()(
  "ConvexService",
  {
    effect: Effect.gen(function* () {
      ...
      return new ConvexServiceImpl(config);
    }),
...
/**
 * Legacy export for ConvexServiceTag (for backwards compatibility during migration)

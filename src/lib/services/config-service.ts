    ...
    const config: AppConfig = {
      mapbox: {
        token: yield* Config.string("MAPBOX_ACCESS_TOKEN").pipe(
          ...
          Config.withDescription("Mapbox access token for server-side geocoding and mapping services")
        ),
        publicToken: yield* Config.string("NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN").pipe(
          Config.withDefault("pk.test"),
          Config.withDescription("Mapbox public access token for client-side map rendering")
        ),
      },
      exa: {
        apiKey: yield* Config.string("EXA_API_KEY").pipe(
          Config.withDefault(""),
          Config.withDescription("Exa API key for semantic search - server-side only, never expose to client")
        ),
      },
      lta: {
        accountKey: yield* Config.string("LTA_ACCOUNT_KEY").pipe(
          Config.withDefault(""),
          Config.withDescription("LTA DataMall AccountKey for API access - server-side only, never expose to client")
        ),
      },
      convex: {
        deployment: yield* Config.string("CONVEX_DEPLOYMENT").pipe(
          Config.withDefault(""),
          Config.withDescription("Convex deployment URL for database operations - server-side only")
        ),
        publicUrl: yield* Config.string("NEXT_PUBLIC_CONVEX_URL").pipe(
          Config.withDefault(""),
          Config.withDescription("Convex public URL for client-side database subscriptions")
        ),
      },
      urls: {
...
export const mapboxTokenConfig = Config.string("MAPBOX_ACCESS_TOKEN").pipe(
  Config.withDefault("pk.test"),
  Config.withDescription("Mapbox access token for geocoding and mapping services")
);
